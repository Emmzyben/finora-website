...
const VenmoTransferPage = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!form.recipient_account || !form.recipient_name || !form.pin || Number(form.amount) <= 0) {
            setMessage('Please provide the Venmo username, full name, amount, and transaction PIN.');
            return;
        }
        ...
        try {
            await apiRequest('transfer', {
                method: 'POST',
                body: {
                    transfer_type: 'venmo_transfer',
                    method: 'venmo_transfer',
                    amount: Number(form.amount),
                    currency: 'USD',
                    recipient_name: form.recipient_name,
                    recipient_account: form.recipient_account,
                    description: form.note || 'Venmo transfer',
                    phone: form.phone,
                    pin: form.pin,
                },
            });
        ...
        <div className="dashboard-content w-100 pb-5">
            ...
            <div className="card border-0 mb-4 text-white text-center position-relative overflow-hidden" style={{ borderRadius: '16px', background: 'linear-gradient(135deg, #023888 0%, #1a56db 100%)', minHeight: '200px' }}>
                ...
                <div className="card-body py-5 d-flex flex-column justify-content-center align-items-center position-relative" style={{ zIndex: 1 }}>
                    <div className="mb-3 d-flex align-items-center justify-content-center rounded-circle" style={{ width: '64px', height: '64px', backgroundColor: 'rgba(255,255,255,0.15)' }}>
                        <span className="fw-bold" style={{ color: '#fff', fontSize: '18px' }}>venmo</span>
                    </div>
                    <h4 className="fw-bold text-white mb-2">Venmo Withdrawal</h4>
                    <p className="text-white-50 mb-0">Funds will be transferred to your Venmo account within 24 hours.</p>
                </div>
            </div>
            ...
            <div className="card border-0 shadow-sm rounded-3 p-4 p-md-5 mb-4">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        <label className="form-label text-dark small mb-1">Venmo Username</label>
                        <div className="input-group">
                            ...
                            <input type="text" name="recipient_account" className="form-control border-start-0 ps-0" placeholder="Enter Venmo username" value={form.recipient_account} onChange={handleChange} />
                        </div>
                    </div>
                    ...
                    <div className="mb-4">
                        ...
                        <div className="input-group">
                            ...
                            <input type="text" name="phone" className="form-control border-start-0 ps-0" placeholder="Enter phone number associated with Venmo" value={form.phone} onChange={handleChange} />
                        </div>
                    </div>
...
export default VenmoTransferPage;
