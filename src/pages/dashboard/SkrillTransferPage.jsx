...
import { ArrowLeft, Lock, Eye, EyeOff, MessageSquare, Send, ShieldCheck, Mail, User } from 'lucide-react';
import { apiRequest } from '../../lib/api';

const SkrillTransferPage = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!form.recipient_account || !form.recipient_name || !form.pin || Number(form.amount) <= 0) {
            setMessage('Please provide the Skrill email, full name, amount, and transaction PIN.');
            return;
        }
        ...
        try {
            await apiRequest('transfer', {
                method: 'POST',
                body: {
                    transfer_type: 'skrill_transfer',
                    method: 'skrill_transfer',
                    amount: Number(form.amount),
                    currency: 'USD',
                    recipient_name: form.recipient_name,
                    recipient_account: form.recipient_account,
                    description: form.note || 'Skrill transfer',
                    pin: form.pin,
                },
            });
        ...
        <div className="dashboard-content w-100 pb-5">
            ...
            <div className="card border-0 mb-4 text-white text-center position-relative overflow-hidden" style={{ borderRadius: '16px', background: 'linear-gradient(135deg, #023888 0%, #1a56db 100%)', minHeight: '200px' }}>
                ...
                <div className="card-body py-5 d-flex flex-column justify-content-center align-items-center position-relative" style={{ zIndex: 1 }}>
                    <div className="mb-3 d-flex align-items-center justify-content-center rounded-circle" style={{ width: '64px', height: '64px', backgroundColor: 'rgba(255,255,255,0.15)' }}>
                        <span className="fw-bold" style={{ color: '#8b2046', fontSize: '24px' }}>Skrill</span>
                    </div>
                    <h4 className="fw-bold text-white mb-2">Skrill Withdrawal</h4>
                    <p className="text-white-50 mb-0">Withdrawals to Skrill are processed within 24 hours.</p>
                </div>
            </div>
            ...
            <div className="card border-0 shadow-sm rounded-3 p-4 p-md-5 mb-4">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        <label className="form-label text-dark small mb-1">Skrill Email</label>
                        <div className="input-group">
                            ...
                            <input type="email" name="recipient_account" className="form-control border-start-0 ps-0" placeholder="Enter Skrill email address" value={form.recipient_account} onChange={handleChange} />
                        </div>
                    </div>
...
export default SkrillTransferPage;
