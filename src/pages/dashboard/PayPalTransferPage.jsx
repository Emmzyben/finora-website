...
import { ArrowLeft, Lock, Eye, EyeOff, MessageSquare, Send, ShieldCheck, Mail } from 'lucide-react';
import { apiRequest } from '../../lib/api';

const PayPalTransferPage = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!form.recipient_email || !form.pin || Number(form.amount) <= 0) {
            setMessage('Please provide the recipient email, amount, and transaction PIN.');
            return;
        }
        ...
        try {
            await apiRequest('transfer', {
                method: 'POST',
                body: {
                    transfer_type: 'paypal_transfer',
                    method: 'paypal_transfer',
                    amount: Number(form.amount),
                    currency: 'USD',
                    recipient_email: form.recipient_email,
                    description: form.note || 'PayPal transfer',
                    pin: form.pin,
                },
            });
    ...
    return (
        <div className="dashboard-content w-100 pb-5">
            ...
            <div
                ...
                style={{ borderRadius: '16px', background: 'linear-gradient(135deg, #023888 0%, #1a56db 100%)', minHeight: '200px' }}
            >
                {/* Back button */}
                <button
                    onClick={() => navigate('/dashboard/internationaltransfer')}
                    className="position-absolute top-0 start-0 m-3 btn btn-sm text-white d-flex align-items-center justify-content-center"
                    style={{ width: '36px', height: '36px', borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.2)', border: 'none', zIndex: 2 }}
                >
                    <ArrowLeft size={18} />
                </button>
                ...
                <div className="card-body py-5 d-flex flex-column justify-content-center align-items-center position-relative" style={{ zIndex: 1 }}>
                    <div
                        ...
                        style={{ width: '64px', height: '64px', backgroundColor: 'rgba(255,255,255,0.15)' }}
                    >
                        <svg viewBox="0 0 24 24" width="36" height="36" fill="none">
                            <text x="50%" y="55%" dominantBaseline="middle" textAnchor="middle" fill="white" fontSize="16" fontWeight="bold" fontStyle="italic">P</text>
                        </svg>
                    </div>
                    <h4 className="fw-bold text-white mb-2">PayPal Withdrawal</h4>
                    <p className="text-white-50 mb-0">Funds will be sent to your PayPal account within 24 hours.</p>
                </div>
            </div>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        ...
                        <div className="d-flex align-items-center px-3 py-2 rounded-3 border" style={{ borderColor: '#6c8cf8', borderWidth: '2px', backgroundColor: '#f5f8ff' }}>
                            ...
                            />
                            <div className="d-flex flex-column align-items-center me-2 opacity-50" style={{ cursor: 'pointer' }}>
                    ...
                    {/* PayPal Email */}
                    <div className="mb-4">
                        <label className="form-label text-dark small mb-1">PayPal Email</label>
                        <div className="input-group">
                            <span className="input-group-text bg-white border-end-0 text-muted"><Mail size={16} style={{ opacity: 0 }} />
                                <div className="position-absolute start-0 ms-3"><Mail size={16} /></div>
                            </span>
                            <input type="email" name="recipient_email" value={form.recipient_email} onChange={handleChange} className="form-control border-start-0 ps-0" placeholder="Enter PayPal email address" />
                        </div>
                        <small className="text-muted mt-1 d-block">Please ensure this is the email address associated with your PayPal account</small>
                    </div>
                    ...
                    <div className="mb-4">
                        ...
                        <div className="input-group">
                            <span className="input-group-text bg-white border-end-0 text-muted"><Lock size={16} style={{ opacity: 0 }} />
                                <div className="position-absolute start-0 ms-3"><Lock size={16} /></div>
                            </span>
                            <input
                    ...
                    <div className="mb-5">
                        ...
                        <div className="input-group align-items-start">
                            <span className="input-group-text bg-white border-end-0 text-muted pt-2 border-primary" style={{ borderWidth: '2px 0 2px 2px' }}>
                            ...
                            <textarea
                                ...
                                value={form.note}
                                onChange={handleChange}
                                className="form-control border-start-0 ps-0 border-primary"
                                rows="4"
...
export default PayPalTransferPage;
