...
import { ArrowLeft, Lock, Eye, EyeOff, MessageSquare, Send, ShieldCheck, Mail, User, MapPin } from 'lucide-react';
import { apiRequest } from '../../lib/api';

const WiseTransferPage = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!form.recipient_name || !form.recipient_account || !form.pin || Number(form.amount) <= 0) {
            setMessage('Please provide the recipient details, amount, and transaction PIN.');
            return;
        }
        ...
        try {
            await apiRequest('transfer', {
                ...
                body: {
                    ...
                    description: form.note || 'Wise transfer',
                ...
                <div className="card-body py-5 d-flex flex-column justify-content-center align-items-center position-relative" style={{ zIndex: 1 }}>
                    <div className="mb-3 d-flex align-items-center justify-content-center rounded-circle" style={{ width: '64px', height: '64px', backgroundColor: 'rgba(255,255,255,0.15)' }}>
                        <svg viewBox="0 0 24 24" width="36" height="36">
                            <path d="M7 19.5l3.5-9.5H4l13-8-3.5 9.5H19l-12 8z" fill="white" />
                        </svg>
                    </div>
                    <h4 className="fw-bold text-white mb-2">Wise Transfer Withdrawal</h4>
                    <p className="text-white-50 mb-0">Your funds will be processed within 1-2 business days.</p>
                </div>
            </div>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="row g-3 mb-4">
                        ...
                        <div className="col-md-6">
                            ...
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><Mail size={16} style={{ opacity: 0 }} /><div className="position-absolute start-0 ms-3"><Mail size={16} /></div></span>
                                <input type="email" name="recipient_account" className="form-control border-start-0 ps-0" placeholder="Enter your email address" value={form.recipient_account} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="col-12">
                            <label className="form-label text-dark small mb-1">Country</label>
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><MapPin size={16} style={{ opacity: 0 }} /><div className="position-absolute start-0 ms-3"><MapPin size={16} /></div></span>
                                <input type="text" name="country" className="form-control border-start-0 ps-0" placeholder="Afghanistan" value={form.country} onChange={handleChange} />
                            </div>
                        </div>
                    </div>
...
export default WiseTransferPage;
