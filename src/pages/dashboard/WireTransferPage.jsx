...
import { ArrowLeft, User, Hash, Building2, MapPin, Globe, Code2, CreditCard, Lock, Eye, EyeOff, MessageSquare, Send, ShieldCheck } from 'lucide-react';
import { apiRequest } from '../../lib/api';

const countries = [
    'Afghanistan', 'Albania', 'Algeria', 'Argentina', 'Australia', 'Austria',
    'Belgium', 'Brazil', 'Canada', 'China', 'Denmark', 'Egypt', 'Finland',
...
const WireTransferPage = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!form.recipient_name || !form.recipient_account || !form.bank_name || !form.pin || Number(form.amount) <= 0) {
            setMessage('Please complete the beneficiary information, amount, and transaction PIN.');
            return;
        }
        ...
        try {
            await apiRequest('transfer', {
                ...
                body: {
                    ...
                    description: form.note || 'International wire transfer',
                    pin: form.pin,
        ...
        <div className="dashboard-content w-100 pb-5">
            ...
            <div className="card border-0 mb-4 text-white text-center position-relative overflow-hidden" style={{ borderRadius: '16px', background: 'linear-gradient(135deg, #023888 0%, #1a56db 100%)', minHeight: '200px' }}>
                ...
                <div className="card-body py-5 d-flex flex-column justify-content-center align-items-center position-relative" style={{ zIndex: 1 }}>
                    <h4 className="fw-bold text-white mb-2">International Wire Transfer</h4>
                    <p className="text-white-50 mb-0">Funds will reflect in the Beneficiary Account within 72hours.</p>
                </div>
            </div>
            ...
            <div className="card border-0 shadow-sm rounded-3 p-4 p-md-5 mb-4">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        ...
                        <div className="d-flex align-items-center px-3 py-2 rounded-3 border border-primary" style={{ borderWidth: '2px' }}>
                            <span className="text-muted fw-bold me-2 fs-5">$</span>
                            <input type="number" name="amount" className="form-control border-0 shadow-none p-0 fw-bold fs-4 text-dark" value={form.amount} onChange={handleChange} />
                    ...
                    <div className="row g-3 mb-3">
                        <div className="col-md-6">
                            <label className="form-label text-dark small mb-1">Beneficiary Account Name</label>
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><User size={16} /></span>
                                <input type="text" name="recipient_name" className="form-control border-start-0 ps-0" placeholder="Enter beneficiary's full name" value={form.recipient_name} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="col-md-6">
                            <label className="form-label text-dark small mb-1">Beneficiary Account Number</label>
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><Hash size={16} /></span>
                                <input type="text" name="recipient_account" className="form-control border-start-0 ps-0" placeholder="Enter account number" value={form.recipient_account} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="col-md-6">
                            <label className="form-label text-dark small mb-1">Bank Name</label>
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><Building2 size={16} /></span>
                                <input type="text" name="bank_name" className="form-control border-start-0 ps-0" placeholder="Enter bank name" value={form.bank_name} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="col-md-6">
                            <label className="form-label text-dark small mb-1">Bank Address</label>
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><MapPin size={16} /></span>
                                <input type="text" name="bank_address" className="form-control border-start-0 ps-0" placeholder="Enter bank address" value={form.bank_address} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="col-md-4">
                            <label className="form-label text-dark small mb-1">Account Type</label>
                            <div className="input-group">
                                ...
                                <select name="account_type" value={form.account_type} onChange={handleChange} className="form-select border-start-0 ps-1">
                        ...
                        <div className="col-md-4">
                            <label className="form-label text-dark small mb-1">Country</label>
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><Globe size={16} /></span>
                                <select name="country" value={form.country} onChange={handleChange} className="form-select border-start-0 ps-1">
                                    {countries.map((c) => <option key={c} value={c}>{c}</option>)}
                                </select>
                            </div>
                        </div>
                        <div className="col-md-4">
                            <label className="form-label text-dark small mb-1">Swift Code</label>
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><Code2 size={16} /></span>
                                <input type="text" name="swift_code" className="form-control border-start-0 ps-0" placeholder="Enter SWIFT/BIC code" value={form.swift_code} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="col-12">
                            <label className="form-label text-dark small mb-1">IBAN Number</label>
                            <div className="input-group">
                                <span className="input-group-text bg-white border-end-0 text-muted"><CreditCard size={16} /></span>
                                <input type="text" name="iban" className="form-control border-start-0 ps-0" placeholder="Enter IBAN number" value={form.iban} onChange={handleChange} />
                            </div>
                            <small className="text-muted mt-1 d-block">International <span className="text-primary">Bank Account Number</span> - Format varies by country</small>
                        </div>
                    </div>
                    ...
                    <div className="mb-5">
                        ...
                        <div className="input-group align-items-start">
                            ...
                            <textarea name="note" className="form-control border-start-0 ps-0" rows="4" placeholder="Optional payment description or note" value={form.note} onChange={handleChange} />
                        </div>
                    </div>
